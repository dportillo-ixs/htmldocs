...
test.describe("CLI Login Flow", () => {
  ...
  test("complete login flow with team selection", async () => {
    ...
    const cliPath = path.resolve(
      __dirname,
      "../../../packages/htmldocs/dist/cli/index.mjs",
    );
    ...
    await authPage
      .getByRole("button", { name: /create new api token/i })
      .click();

    // Wait for success message
    await expect(
      authPage.getByText("Token Created Successfully"),
    ).toBeVisible();
    await expect(
      authPage.getByText("You can safely close this window now."),
    ).toBeVisible();
