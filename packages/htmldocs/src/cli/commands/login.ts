...
export const login = async (options: LoginOptions = {}) => {
  ...
  try {
    ...
    if (!sessionResponse.ok) {
      ...
      console.log(
        chalk.red("Failed to initialize authentication:", error.message),
      );
      process.exit(1);
    }
    ...
    if (options.headless) {
      ...
      console.log(
        chalk.blue(
          "Please select a team and complete the authentication in your browser.",
        ),
      );
    }
    ...
    while (true) {
      ...
      try {
        const response = await fetch(
          `${apiUrl}/api/auth/check-status?session_id=${sessionId}`,
        );
        ...
        console.log(
          chalk.red("Error checking authentication status. Please try again."),
        );
        process.exit(1);
      }
    }
